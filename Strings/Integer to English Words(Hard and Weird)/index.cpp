...
string helper(int n)
{
    ...
    if(n>=20) return " "+ten[n/10-2]+helper(n%10);  //starts from twenty
    
    if(n>=1) return " "+one[n-1]; //starts from one
...
string numberToWords(int num) {
    ...
    if(num==0)
        return "Zero";
    //returning the full ans bcoz we are using + operator....
    return helper(num).substr(1); //" Million"," Billion" remove first space....2nd character to end.....
}
};