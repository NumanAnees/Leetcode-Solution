...
public:
    int merge(vector<int> &nums, int l, int m, int h) {
        ...
        int j=m+1;
        for(int i=l;i<=m;i++) {
            while(j<=h && nums[i] > 2LL*nums[j]) j++;//The LL makes the integer literal of type long long
            count += (j - (m+1)); //how many locations j moved from its initial position.
        }
        ...
        for(int i=l;i<=h;i++) {
            nums[i]=temp[i-l];//changing original array
        }
        return count;
    }
